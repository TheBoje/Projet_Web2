...
#
reps="config public sqlite src templates"
for rep in $reps
do
...
# base de données
#
reponse=`tar zvtf $filename --no-recursion --wildcards 'PROJET/site/sqlite/*.db' 2> /dev/null`
if [ ! "$reponse" ]
then
    echo "[ERR]  pas de fichier PROJET/site/sqlite/*.db dans l'archive"
    exit
else
